...
# sobolSalt = sensitivity of Sobol in the R
#attributes(sens_P)
...
compute_pop_metrics <- function(result, thresh) {
  ...
  #findout when the population is greater han some threshold (which rows in my dataframe is greater than the threshold)
...
Pinitial=10
# run for 500 days
simtimes = seq(from=1, to=500, by=5) #simulation times
...
parmtest = list(r=sens_P$X[1,"r"],K=sens_P$X[1,"K"]) #first parameter set of Sobol
...
#gives maximum population and the threshold year
...
allresults <- as.data.frame(sens_P$X) %>%
  pmap(p_wrapper, Pinitial = Pinitial, simtimes = simtimes,
       odefunc = dpopgrowth, metricfunc = compute_pop_metrics,
       metricpars=list(thresh=100))
# run function for every rows in the function/ pmap runs the function over and over again
...
sens_P_threshyear$S # run in console

# total sensitivity index -note that this partitions the output variance - so values sum to 1
rownames(sens_P_threshyear$T) <- c("r", "K")
sens_P_threshyear$T # run in console
...
# result = the first-order indices and total sensitivity index are quiet different, so they are interacting
