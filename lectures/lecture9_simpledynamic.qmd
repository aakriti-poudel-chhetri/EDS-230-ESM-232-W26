...
Dynamic modeling is about predicting how things change over time and across space. What happens in one spot depends on what's around it, and what happens next depends on what happened before. Time only moves forward, but spatial effects go in all directions. These models get complicated fast, which is why engineers spend a lot of time studying them.
...
-   Diffusion (anything that is diffusive is dynamic in time)
-   Population
...
-   *chaotic* ..highly sensitive to initial conditions (chaos in environment modelling is the highly sensitive model)
